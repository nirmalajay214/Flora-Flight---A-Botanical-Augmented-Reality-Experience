...
public class VegetationOnMesh: MonoBehaviour
{
  ...
  private void Update()
  {
    ...
    if (vertexCount >= _spawnMinVertexCount && !(bool)_plant)
    {
      ...
      if(_plant.name == "PP_Daffodil_03(Clone)" || _plant.name == "PP_Hyacinth_04(Clone)" || _plant.name == "PP_Mushroom_Fantasy_Orange_10(Clone)" ){
        // Debug.Log(_plant.tag);
          // objectList.Add((_plant) as GameObject);
      }
    }
